...
class Utility(commands.Cog):
    ...
    async def setprefix(self, ctx,*,prefix='.'):
        await self.client.pg_con.execute("UPDATE guild SET prefix = $1 WHERE guildid = $2",prefix,ctx.guild.id)
    ...
    async def setsuggestion(self,ctx,channel:discord.TextChannel):
        ...
        prefix = prefix[0]
        await self.client.pg_con.execute("UPDATE guild SET suggestionchannel = $1 WHERE guildid = $2",channel.id,ctx.guild.id)
        embed = discord.Embed(title = "Suggestion Channel Updated",description = f"Suggestion channel has been set to {channel.mention}\nNow Type `{prefix}suggest + [suggestion]` without these '[',']' to Send a suggestion into the channel and start a Poll",color = random.choice(colors))
        await ctx.send(embed = embed)
    ...
    async def remove_log(self,ctx):
        prefix = await self.client.pg_con.fetchrow("SELECT prefix FROM guild WHERE guildid = $1",ctx.guild.id)
        prefix = prefix[0]
        await self.client.pg_con.execute("UPDATE guild SET logchannel = $1 WHERE guildid = $2",None,ctx.guild.id)
        await ctx.send(embed = discord.Embed(title = "Logging Turned Off",description = f"Logging Has been Deactivated, This server won't be updated with the user's activity, status, roles, nickname anymore\nType `{prefix}set_logs #log_channel` to start enojoying the Features again ",color = random.choice(colors)))
    ...
    async def remove_sug(self,ctx):
        prefix = await self.client.pg_con.fetchrow("SELECT prefix FROM guild WHERE guildid = $1",ctx.guild.id)
        prefix = prefix[0]
        await self.client.pg_con.execute("UPDATE guild SET suggestionchannel = $1 WHERE guildid = $2",None,ctx.guild.id)
        await ctx.send(embed = discord.Embed(title = "Suggestion Command Turned Off",description = f"Suggestions Have been Deactivated, You won't be able to use Suggestion command in 'tis server now\nType `{prefix}set_suggestion #suggestion_channel` to start enojoying the Command again",color = random.choice(colors)))
    ...
    async def remove_aibot(self,ctx):
        prefix = await self.client.pg_con.fetchrow("SELECT prefix FROM guild WHERE guildid = $1",ctx.guild.id)
        prefix = prefix[0]
        await self.client.pg_con.execute("UPDATE guild SET aichannel = $1 WHERE guildid = $2",None,ctx.guild.id)
        await ctx.send(embed = discord.Embed(title = "AI-Chatbot Turned Off",description = f"AI-chabot features Have been Deactivated, You won't be able to enjoy Chatbot Features now\nType `{prefix}set_aichatbot #Aichatbot_channel` to start enojoying the features again",color = random.choice(colors)))
    ...
    async def remove_main (self,ctx):
        prefix = await self.client.pg_con.fetchrow("SELECT prefix FROM guild WHERE guildid = $1",ctx.guild.id)
        prefix = prefix[0]
        await self.client.pg_con.execute("UPDATE guild SET mainchannel = $1 WHERE guildid = $2",None,ctx.guild.id)
        await ctx.send(embed = discord.Embed(title = "Main-Channel features Turned Off",description = f"Now you won't be getting any welcome commands, leave-join events, drops etc\nType `{prefix}set_main #main_name` to enjoy these features again ",color = random.choice(colors)))

    @commands.command(aliases = ['invite'])
    async def join(self,ctx):
        embed = discord.Embed(title = "Invite",timestamp = datetime.datetime.utcnow(),color = random.choice(colors))
        ...
        owner = get(self.client.users , id = self.client.owner_id)
