...
shopping = [{"name":"demonslayer","price" : 120000 , "description" : "Set this Anime Background in your Profile by `.set + demonslayer` Click [HERE](https://i.pinimg.com/originals/a3/19/db/a319db489eb10c63d34f286d6d81008d.jpg) for Image Preview" } ,
            {"name":"warzone","price" : 100000 , "description" : "Set this Gaming Background in your Profile by `.set + warzone` Click [HERE](https://wallpaperaccess.com/full/2541966.jpg) for Image Preview" } ,
            {"name":"maskcodes","price" : 20000 , "description" : "Set this Coding Background in your Profile by `.set + maskcodes` Click [HERE](https://www.teahub.io/photos/full/87-879470_computer-programming-coding-technology.jpg) for Image Preview" },
            {"name":"mikasa","price" : 199999 , "description" : "Set this Anime Background in your Profile by `.set + mikasa` Click [HERE](https://i.pinimg.com/originals/a3/19/db/a319db489eb10c63d34f286d6d81008d.jpg) for Image Preview" } ,
            {"name":"devilmay","price" : 40000 , "description" : "Set this Gaming Background in your Profile by `.set + devilmay` Click [HERE](https://wallpaperaccess.com/full/2541966.jpg) for Image Preview" } ,
            {"name":"binary","price" : 100000 , "description" : "Set this Coding Background in your Profile by `.set + binary` Click [HERE](https://www.teahub.io/photos/full/87-879470_computer-programming-coding-technology.jpg) for Image Preview" } ]
...
class Economy(commands.Cog):
    ...
    async def invent(self,id):
        inventory = await self.client.pg_con.fetchrow("SELECT inventory FROM users WHERE userid = $1",id)
        return inventory[0]
    ...
    async def donate(self,ctx,member:discord.Member,amount = None):
        ...
        if await self.get(ctx.author.id)>= amount:
            ...
            embed = discord.Embed(title="Credits Transfered",description = f"{ctx.author.name} Have donated {amount} to {member.mention}",color = random.choice(colors))
    ...
    @commands.command()
    @commands.is_owner()
    async def resetall(self,ctx,amount:int=2000):
        await self.client.pg_con.execute("UPDATE users SET credits = $1",amount)
        await ctx.send("Done")
        
    @commands.command()
    @commands.is_owner()
    async def reward(self,ctx,member:discord.Member,amount:int = 10000):
        em = discord.Embed(title = f"Congratulations {member.name} !!! ",description = f"{member.mention} is rewarded {amount} credits ",color = random.choice(colors))
...
""",color = random.choice(colors))
            ...
            embed.add_field(name = f"**{self.client.user.name.title()}** ",value=f"Strikes `{botstrike}`")
...
""",color = random.choice(colors))
    ...
    async def earn(self,ctx):
        ...
        prefix = prefix[0]
        em = discord.Embed(title = "Ways To Earn : ",color = random.choice(colors))
    ...
    async def work(self,ctx):
        ...
        em = discord.Embed(title ="Work Results : ",description = f"{ctx.author.mention} has Earned {credits} Credits through Work :)\nTotal Credits : {money}",color = random.choice(colors))
        em.set_author(name = self.client.user.name,url = self.client.user.avatar_url)
        em.timestamp = datetime.datetime.utcnow()
        await ctx.send(embed = em)
    ...
    async def picking(self):
        ...
        embed = discord.Embed(title = f"Someone Dropped their {thing} {thing.replace(':','')} and some Credits",description = "Type `pick` to Add it to you inventory and then sell it for Credits", color=random.choice(colors))
        embed.timestamp = datetime.datetime.utcnow()
        msg = await channel.send(embed = embed)
        try:
            authormsg = await self.client.wait_for('message', timeout=600.0, check=check)
            ...
            await channel.send(embed=discord.Embed(description=f"{author.name} Got {thing} {thing.replace(':','')} and {amount} Credits", color=random.choice(colors)))
    ...
    async def credits(self,ctx):
        await self.check(ctx.author.id)
        credits =  await self.get(ctx.author.id)
        em = discord.Embed(title = "Credits Info : ",color = random.choice(colors))
    ...
    async def slots(self,ctx,amount = None):
        ...
        if await self.get(ctx.author.id)>= amount:
            for i in range(3):
                a = random.choice([":coin:",":money_mouth:",":moneybag:",":money_with_wings:",":coin:",":money_mouth:",":moneybag:",":money_with_wings:"])
    ...
    async def shop(self, ctx):
        ...
        prefix = await self.client.pg_con.fetchrow("SELECT prefix FROM guild WHERE guildid = $1",ctx.guild.id)
        prefix = prefix[0]
        em = discord.Embed(title = "Buy Items",description = "Type `.buy + itemname` to buy an Item",color = random.choice(colors))
    ...
    async def beg(self,ctx):
        ...
        if amount == 0:
            ...
            await ctx.send(embed = discord.Embed(title = "Woow!!",description = f"Seems like {person.title()} donated you {amount} and {ranstuff} :star_struck: !! ",color = random.choice(colors)))
        elif amount <500 and amount >100:
            await ctx.send(embed = discord.Embed(title = "Nice!!",description = f"You got mediocre amount of {amount} and a {ranstuff} from {person.title()} :) !! ",color = random.choice(colors)))
        elif amount <100:
            await ctx.send(embed = discord.Embed(title = "Ahhh!",description = f"{person.title()} donated you a small portion of {amount} and a {ranstuff} :expressionless: !! ",color = random.choice(colors)))
        await self.plus(ctx.author.id,mystuff)

    @commands.command(aliases = ['inven'])
    async def inventory(self,ctx,member:discord.Member = None):
        ...
        embed = discord.Embed(title = "Inventory" ,color = random.choice(colors))
        if data is not None:
    ...
    async def sell(self,ctx,item):
        ...
        if item.lower() == "all":
            ...
            if not items == 0:
                ...
                msg1 = discord.Embed(title = "Offer",description = f"{name} is offering you {amount} for All your Items",color = random.choice(colors))
                ...
                await msg1.add_reaction("❎")
                try:
                    reaction = await self.client.wait_for('reaction_add',timeout = 20.0,check = check)
                except asyncio.TimeoutError:
        ...
        if item in inventory:
            ...
            msg1 = discord.Embed(title = "Offer",description = f"{name} is offering you {amount} for {item} {item.replace(':','')}",color = random.choice(colors))
            msg1.set_footer(text = "Rates of Items Keeps on Fluctuating, You can Decline and maybe Try again Later")
            msg1.set_author(name = f"Offer for {ctx.author.name}",icon_url=ctx.author.avatar_url)
            msg1.set_thumbnail(url="https://www.clipartkey.com/mpngs/m/126-1261730_suit-clipart-anonymous-anonymous-person-silhouette.png")
            msg1 = await ctx.send(embed = msg1)
            ...
            if item == back['name']:
                if balance>back['price']:
                    if item not in backs:
                        embed = discord.Embed(title = "Background for Profile Bought",description = f"{ctx.author.name} have succesfully Bought the {item.title()} Background Role ",color = random.choice(colors))
    ...
    async def hunt(self,ctx):
        ...
        if animal != 0:
            embed = discord.Embed(title = "Hunting Results",description = f"Nice Play , You hunted down a {animal} {animal.replace(':','')}",color = random.choice(colors))
            
            await self.plus(ctx.author.id,animal)
        else:
            embed = discord.Embed(title = "Hunting Results",description = "Ah You are trash at hunting, You found nothing, Better do a bit more Practice",color = random.choice(colors))
    ...
    async def fish(self,ctx):
        ...
        if fish != 0:
            embed = discord.Embed(title = "Fishing results",description = f"Very well done , you captured a {fish} {fish.replace(':','')}",color = random.choice(colors))
            await self.plus(ctx.author.id,fish)
        else:
            embed = discord.Embed(title = "Fishing results",description = f"huh, What was that!? , You found nothing :confused:",color = random.choice(colors))
        embed.set_author(name=ctx.author.name,icon_url=ctx.author.avatar_url)
        embed.set_footer(text = "Type .inventory to see you inventory")
        embed.timestamp = datetime.datetime.utcnow()
        await ctx.send(embed = embed)
