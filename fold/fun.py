...
class Fun(commands.Cog):
    ...
    async def coffee(self, ctx, member: discord.Member ):
            if member == ctx.author:
                await ctx.send(embed=discord.Embed(description=f"☕ | {ctx.author.name} Enjoying Coffee alone :smirk: ", color=random.choice(colors)))
                return
            elif member == self.client.user:
                await ctx.send(embed=discord.Embed(description="☕ | Don't worry I will drink coffee with you  *sips*", color=random.choice(colors)))
                return
            elif member.bot: return;

            coffee_offer = f"☕ | {member.mention}, you got a coffee offer from {ctx.author.name}"
            coffee_msg = await ctx.send(embed=discord.Embed(description=coffee_offer,color=random.choice(colors)))
            ...
            try:
                ...
                await ctx.send(embed=discord.Embed(description=f"Looks like {member.name} are busy", color=random.choice(colors)))
            else:
                await coffee_msg.delete()
                await ctx.send(embed=discord.Embed(description=f"☕ | Yay! {ctx.author.name} and {member.name} are enjoying coffee together!", color=random.choice(colors)))
    @commands.command(aliases = ['drink'])
    async def beer(self, ctx, member: discord.Member ):
            if member == ctx.author:
                await ctx.send(embed=discord.Embed(description=f"🍺 | {ctx.author.name} Party Time !! , *Enjoing Beer*", color=random.choice(colors)))
                return
            elif member == self.client.user:
                await ctx.send(embed=discord.Embed(description="🍺 | Don't worry I will Enjoy beer with you  *brrr*", color=random.choice(colors)))
                return
            elif member.bot: return;
            ...
            try:
                ...
                await ctx.send(embed=discord.Embed(description=f"Looks like {member.name} is busy", color=random.choice(colors)))
            else:
                await coffee_msg.delete()
                await ctx.send(embed=discord.Embed(description=f"🍺 | Yay! {ctx.author.name} and {member.name} are enjoying Beer Party!", color=random.choice(colors)))
